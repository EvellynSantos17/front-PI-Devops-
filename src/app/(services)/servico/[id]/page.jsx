      ...
      <div className="w-full max-w-[1168px] bg-white px-8 pt-5 pb-10 rounded-t-3xl flex items-center justify-center flex-col">
        <div className=" w-full bg-gradient-to-r from-laranjaProdunfo h-fit p-2 to-orange-300 flex items-center rounded-full justify-between">
          <button className="w-fit h-fit" onClick={() => router.back()}>
            <Image
              className="p-2 rounded-full bg-white hover:bg-zinc-100 "
              alt=""
              src={"/icons/_-.png"}
              width={45}
              height={45}
          ...
          <ul className="flex w-full items-center h-fit flex-wrap justify-start gap-2">
            {service.skills.map((item, index) => {
              return (
                <li
                  key={index}
                  className="px-5 py-1 rounded-full bg-[#FFD0B0] text-laranjaProdunfo shadow-md font-semibold"
                >
                  {item}
                </li>
              );
        ...
        <div className="w-full py-4">
          ...
          <button
            ...
            disabled={matchId ? true : false}
            className={`${
              matchId
                ? "bg-zinc-500 cursor-not-allowed"
                : "bg-laranjaProdunfo hover:bg-opacity-85 "
            }  w-full flex gap-2 py-2 text-xl items-center text-white justify-center font-bold rounded-xl`}
          >
            <Image
              alt="perfil"
            ...
            {service.contractedListings &&
              service.contractedListings.map((contracted, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="max-w-[330px] max-h-[200px] w-full h-full p-3 border border-laranjaProdunfo hover:scale-105 transition-all rounded-2xl"
                  >
                    <div className="flex gap-1 items-center p-1 max-w-[300px] w-full border border-laranjaProdunfo rounded-full ">
                      <Image
                        className="rounded-full"
                      ...
                      <div className="pr-10 flex flex-col">
                        ...
                        <p className="text-zinc-400 text-sm ">
                          {contracted.client.title}
                        </p>
                      </div>
                    </div>
                    <div className="flex gap-2 items-center py-2">
                      <Stars
                        numberStar={contracted.evaluation.stars}
                        click={false}
                        size={18}
                      />
