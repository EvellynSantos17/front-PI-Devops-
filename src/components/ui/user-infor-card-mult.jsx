    ...
    return (
        <div className="h-fit flex w-full items-center justify-between p-2 rounded-xl border border-[#00000080] shadow-md">
            <div className="flex flex-col h-full w-full justify-center">
                <h3 className="text-[#32292F8F]">
                    {title}
                </h3>
                
                <ul className="flex gap-2 flex-wrap">
                    {!isEditing && values.map((item, index) => (
                        <li key={index} className="px-5 py-1 bg-laranjaProdunfo text-white rounded-xl flex items-center justify-between">
